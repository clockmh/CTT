...
from ltr.models.neck.transformer_ import build_featurefusion_network
...
class TransT(nn.Module):
    ...
    def forward(self, search, template):
        ...
        outputs_class, outputs_coord = self.featurefusion_network(self.input_proj(src_template).unsqueeze(1), mask_template, self.input_proj(src_search).unsqueeze(1), mask_search)
...
class SetCriterion(nn.Module):
    ...
    def loss_labels(self, outputs, targets, indices, num_boxes, log=True):
        ...
        window = torch.from_numpy(np.outer(hanning, hanning).flatten()).unsqueeze(0).repeat(src_logits.shape[0],1).to(src_logits.device)
        window = torch.stack([window,-window],dim = 1)
        #target_classes = target_classes - window
        #src_logits1 = 0.5*src_logits[...,1] + 0.5*window
        #src_logits2 = F.softmax(src_logits1, dim
        #src_logits[...,1] = src_logits1
        #####
        logsoft_out = F.log_softmax(src_logits.transpose(1, 2),dim = 1)
        
        src_logits_ = 0.5*logsoft_out + 0.5*window
        
        self.empty_weight = torch.tensor(self.empty_weight, dtype=torch.float64).to(src_logits.device)
        loss_ce = F.nll_loss(src_logits_,target_classes,self.empty_weight)
        #####
    ...
    def forward(self, outputs, targets):
        ...
        """
        outputs_without_aux = {k: v for k, v in outputs.items() if k != 'aux_outputs'}#what
